...
const MapWithShapefiles = () => {
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      if (geoJsonData.features.length > MAX_FEATURES) {
        throw new Error(`Shapefile contains too many features (${geoJsonData.features.length}). Maximum allowed: ${MAX_FEATURES}`);
      }
  ...
  useEffect(() => {
    ...
    const renderBatch = async (features, startIndex) => {
      ...
      batch.forEach(feature => {
        try {
          if (feature.geometry.type === 'Polygon' || feature.geometry.type === 'MultiPolygon') {
            ...
            // Simplify complex lines
            const simplified = coordinates.filter((_, index) => index % 2 === 0);
            
            const lineString = new H.map.Polyline(
              simplified,
              {
                style: {
      ...
      // Render next batch
      if (mounted && startIndex + BATCH_SIZE < features.length) {
        setTimeout(() => {
          renderBatch(features, startIndex + BATCH_SIZE);
        }, 50); // Add small delay between batches
      } else if (mounted) {
      ...
      <div className="absolute top-4 right-4 bg-white p-4 rounded-lg shadow-lg z-10 min-w-[200px] max-w-[300px]">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {loading && (
              <div className="absolute top-4 left-1/2 transform -translate-x-1/2 bg-white px-4 py-2 rounded-lg shadow-lg z-20">
                <div className="flex items-center space-x-2">
                  <svg className="animate-spin h-5 w-5 text-blue-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="6" cy="6" r="5" stroke="currentColor" strokeWidth="4"></circle>
                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                  </svg>
                  <span className="text-sm font-medium text-gray-700">Processing shapefile...</span>
                </div>
              </div>
            )}
